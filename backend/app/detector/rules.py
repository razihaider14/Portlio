...
Rule anatomy:
    ...
    evidence_strength : declared / configured / demonstrated; see
        app.detector.models.EvidenceStrength for the full definitions and
        classification guidance. Required on every rule.
    confidence : 0.0 - 1.0; see scale below
    priority : higher = surfaced first in detailed results; see scale below

Evidence strength scale (quick reference; see EvidenceStrength for detail):
    DECLARED      only a dependency-manifest entry (or build-file
                  substring equivalent, e.g. "junit" inside pom.xml) --
                  proves intent, not that the code actually uses it.
    CONFIGURED    a dedicated, non-trivial config file/section or vendored
                  artifact -- deliberate setup beyond a bare dependency
                  line.
    DEMONSTRATED  a real, specific, hand-authored artifact (source file,
                  working CI pipeline, unique config artifact) or a
                  content match confirming actual usage.
...
RULES: list[Rule] = [
    ...
    Rule(
        ...
        category=RuleCategory.EMBEDDED,
        evidence_strength=EvidenceStrength.CONFIGURED,
        confidence=0.9,
        priority=20,
    ),
    Rule(
